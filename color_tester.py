...
for color, (lower, upper) in common_colors.items():
    # Create a 4x4 image of the specified color
    color_image = np.full((50, 50, 3), upper, dtype=np.uint8)

    # Convert the image from HSV to BGR for display
    bgr_color_image = cv2.cvtColor(np.reshape(color_image, (50, 50, 3)), cv2.COLOR_HSV2BGR)
